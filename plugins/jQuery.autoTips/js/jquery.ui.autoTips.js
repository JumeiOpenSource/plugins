                    ...
                    if (key == 13) {//回车
                        ...
                        if (opts.type === 'email') {
                            ...
                            if (txtVal.indexOf('@') != -1) {
                                ...
                                if (postfix != "") {
                                    for (var u = 0; u < opts.dataSource.length; u++) {
                                        if (opts.dataSource[u].indexOf(postfix) != -1) {
                                            _dataSource.push(opts.dataSource[u]);
                                        }
                                    }
                                    _dataSource.unshift(opts.dataSource[0], opts.dataSource[1]);
                                } else if (postfix == "") {
                                    _dataSource = opts.dataSource;
                                } else {
                                    _dataSource.unshift(opts.dataSource[0], opts.dataSource[1]);
                                }
                                _creatTipsElement(_dataSource);
                            } else {
                                _creatTipsElement(opts.dataSource);
                            }
                            //根据输入重构提示内容列表 END
                            li_list = $('#' + opts.tipsID).find('li:not(:first)');
